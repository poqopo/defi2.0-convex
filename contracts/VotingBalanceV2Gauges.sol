...
contract VotingBalanceV2Gauges{
    ...
    function balanceOf(address _account) external view returns(uint256){
        ...
        (, uint32 _date) = ILockedKP(locker).epochs(epochindex);
        ...
        // length -2 = current
        // length -3 = previous
        (, _date) = ILockedKP(locker).epochs(epochindex);
        if(_date >= currentEpoch){
            //if end date is already the current epoch,  minus 1 to get the previous
            epochindex--;
        }
        ...
        //get pending
        uint256 pending = ILockedKP(locker).pendingLockAtEpochOf(epochindex, _account);

        //if using old locker for grace period
        if(UseOldLocker){
            //check if tokens have not been withdrawn yet
            if(ILockedKP(oldlocker).lockedBalanceOf(_account) > 0){
                uint256 eindex = ILockedKP(oldlocker).epochCount() - 1;
                (, uint32 _edate) = ILockedKP(oldlocker).epochs(eindex);
                if(_edate >= currentEpoch){
                    //if end date is already the current epoch,  minus 1 to get the previous
                    eindex--;
                }
                //add to current balance
                pending += ILockedKP(oldlocker).balanceAtEpochOf(eindex, _account);
            }
        }
